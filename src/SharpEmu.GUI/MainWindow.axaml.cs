...
public partial class MainWindow : Window
{
    ...
    private static (string? Title, string? TitleId, string? Version) TryReadParamJson(string ebootPath)
    {
        try
        {
            var paramPath = FindSceSysFile(ebootPath, "param.json");
            if (paramPath is null || !File.Exists(paramPath))
            {
    ...
    private static string? FindCoverFor(string ebootPath)
    {
        ...
        {
            var coverPath = FindSceSysFile(ebootPath, candidate);
    ...
    private static string? FindBackgroundFor(string ebootPath)
    {
        ...
        {
            var backgroundPath = FindSceSysFile(ebootPath, candidate);
    ...
    /// <summary>
    /// Locates a metadata/asset file that games ship under sce_sys/. Dumps
    /// unpack with different nesting: some put the executable next to
    /// sce_sys/, some nest the executable one or more folders deeper while
    /// sce_sys/ stays at the game root, and a few flatten sce_sys/ entirely.
    /// This walks the executable's directory and up to three ancestors,
    /// checking both sce_sys/&lt;file&gt; and a flat &lt;file&gt; in each.
    /// </summary>
    private static string? FindSceSysFile(string ebootPath, string fileName)
    {
        var directory = Path.GetDirectoryName(ebootPath);
        for (var depth = 0; directory is not null && depth < 4; depth++)
        {
            var inSceSys = Path.Combine(directory, "sce_sys", fileName);
            if (File.Exists(inSceSys))
            {
                return inSceSys;
            }
