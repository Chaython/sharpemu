...
var renderTargets = GetRenderTargets(state.CxRegisters);
    ...
    private static GuestDrawTexture CreateFallbackGuestDrawTexture(
        ...
        uint depth = 1)
    {
        // Unbound descriptors have no valid image type. Their other bits must
        // not select a physical format: e.g. a null slot decoded as R8 SSCALED
        // requests an unsupported Vulkan sampled image on NVIDIA hardware.
        // Use a four-byte placeholder, preserving the shader's integer domain.
        if (type < Gen5TextureType1D || type > Gen5TextureType2DArray)
        {
            type = Gen5TextureType2D;
            format = 10;
            numberType = numberType is 4 or 5 ? numberType : 0;
            depth = 1;
        }
