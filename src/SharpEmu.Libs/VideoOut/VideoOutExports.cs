...
public static class VideoOutExports
{
    ...
    private sealed class VideoOutPortState
    {
        ...
        public ulong FlipTimestamp { get; set; }
        public ulong SubmitTimestamp { get; set; }
    ...
    public static int VideoOutGetFlipStatus(CpuContext ctx)
    {
        ...
        Span<byte> status = stackalloc byte[0x40];
        status.Clear();
        lock (_stateGate)
        {
            BinaryPrimitives.WriteUInt64LittleEndian(status, port.FlipCount);
            BinaryPrimitives.WriteUInt64LittleEndian(status[0x08..], port.FlipProcessTime);
            BinaryPrimitives.WriteUInt64LittleEndian(status[0x10..], port.FlipTimestamp);
            BinaryPrimitives.WriteInt64LittleEndian(status[0x18..], port.FlipArg);
            BinaryPrimitives.WriteUInt64LittleEndian(status[0x20..], port.SubmitTimestamp);
            BinaryPrimitives.WriteInt32LittleEndian(status[0x34..], port.PendingFlips);
    ...
    public static int VideoOutIsFlipPending(CpuContext ctx)
    {
        ...
        lock (_stateGate)
        {
            ctx[CpuRegister.Rax] = unchecked((ulong)port.PendingFlips);
            return port.PendingFlips;
        }
    }
        ...
        bool submitGpuImage)
    {
        ...
        lock (_stateGate)
        {
            ...
            port.SubmitTimestamp = KernelRuntimeCompatExports.ReadTimestampCounter();
            port.PendingFlips++;
        ...
        {
            // Publish the completed flip before waking its event consumers.
            // Unity uses the full 64-bit flipArg to release its frame semaphore.
            lock (_stateGate)
            {
                port.CurrentBuffer = bufferIndex;
                port.FlipCount++;
                port.FlipArg = flipArg;
                port.FlipTimestamp = KernelRuntimeCompatExports.ReadTimestampCounter();
                port.FlipProcessTime = KernelRuntimeCompatExports.GetProcessTimeMicroseconds();
                port.PendingFlips--;
            }
