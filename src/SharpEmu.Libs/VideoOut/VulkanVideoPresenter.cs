...
internal static unsafe class VulkanVideoPresenter
{
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        {
            using var profile = RenderPhaseProfile.Measure(RenderPhaseProfile.Phase.Descriptors);
        ...
        private void CreateTranslatedPipeline(
            ...
            Extent2D extent)
        {
            using var profile = RenderPhaseProfile.Measure(RenderPhaseProfile.Phase.Pipeline);
        ...
        {
            using var profile = RenderPhaseProfile.Measure(RenderPhaseProfile.Phase.Texture);
            if (texture.IsStorage)
            {
        ...
        {
            using var profile = RenderPhaseProfile.Measure(RenderPhaseProfile.Phase.GlobalBuffer);
        ...
        {
            using var profile = RenderPhaseProfile.Measure(RenderPhaseProfile.Phase.Geometry);
        ...
        private void WriteBackAllDirtyGuestBuffers(string? queueName = null)
        {
            ...
            foreach (var allocation in _guestBufferAllocations)
            {
                for (var index = allocation.DirtyRanges.Count - 1; index >= 0; index--)
                {
                    ...
                    // A writable descriptor describes potential writes, not byte
                    // ownership. Publish only bytes changed from the upload shadow.
                    // A page-wide read/merge/write would race native CPU stores even
                    // if its merge preserved all unchanged bytes at read time.
                    const int pageSize = 4096;
                    var mappedPageBuffer = GuestDataPool.Shared.Rent(pageSize);
                    try
                    {
                        for (var pageStart = 0; pageStart < mappedBytes.Length; pageStart += pageSize)
                        {
                            var pageLength = Math.Min(pageSize, mappedBytes.Length - pageStart);
                            ...
                            // Avoid repeated scalar reads from write-combined GPU memory.
                            ...
                            while (cursor < pageLength)
                            {
                                ...
                                if (runStart == pageLength)
                                {
                                    break;
                                }

                                var runLength = cursor - runStart;
                                var offset = pageStart + runStart;
                                var changed = mappedPage.Slice(runStart, runLength);
                                changedRuns++;
                                ...
                                var ioStartTicks = _traceGlobalWritebackTiming
                                    ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
                                var writeOk = memory.TryWrite(guestAddress + (ulong)offset, changed);
                                if (_traceGlobalWritebackTiming)
                                {
                                ...
                                if (writeOk)
                                {
                                    changed.CopyTo(shadowBytes.Slice(offset, runLength));
                                    writtenRuns++;
                                    pageWritten = true;
                                }
                                else
                                {
                                    failedRuns++;
                                    MarkGuestBufferDirty(allocation, range.Offset + (ulong)offset,
                                        (ulong)runLength, range.QueueName, range.Timeline);
                                }
                            }
                            if (pageWritten)
                            {
        ...
        {
            using var profile = RenderPhaseProfile.Measure(RenderPhaseProfile.Phase.RetireResources);
