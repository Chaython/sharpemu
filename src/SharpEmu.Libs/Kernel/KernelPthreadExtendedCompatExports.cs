...
public static class KernelPthreadExtendedCompatExports
{
    ...
    public static int PthreadAttrGet(CpuContext ctx)
    {
        ...
        // Ask the allocator for the target pthread's stack before taking the
        // attribute lock. Its address is independent of the caller's current
        // stack (which may belong to a guest callback or another pthread).
        ulong mappedStackAddress = 0, mappedStackSize = 0;
        var hasMappedStack = GuestThreadExecution.Scheduler?.TryGetGuestThreadStack(
            thread, out mappedStackAddress, out mappedStackSize) == true;
        lock (_stateGate)
        {
            ...
            if (hasMappedStack)
            {
                ...
                {
                    StackAddress = mappedStackAddress,
                    StackSize = mappedStackSize,
                };
            }
			...
			else if (thread == KernelPthreadState.GetCurrentThreadHandle() &&
