...
public static class KernelRuntimeCompatExports
{
    ...
    {
        ctx[CpuRegister.Rax] = ReadTimestampCounter();
        return (int)OrbisGen2Result.ORBIS_GEN2_OK;
    }

    internal static ulong ReadTimestampCounter() =>
        TryReadHostTsc(out var counter)
            ? counter
            : unchecked((ulong)Math.Max(0, Stopwatch.GetTimestamp()));
