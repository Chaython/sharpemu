...
public static partial class KernelMemoryCompatExports
{
        ...
        int? memoryType = null)
    {
        ...
        if (!TryProtectHostRange(ctx, alignedAddress, alignedLength, protection))
        {
            if (ShouldTraceDirectMemory())
            {
                Console.Error.WriteLine($"[LOADER][TRACE] mprotect.failed addr=0x{alignedAddress:X} len=0x{alignedLength:X} prot=0x{protection:X}");
                var cursor = alignedAddress;
                for (var index = 0; index < 16 && cursor < alignedAddress + alignedLength; index++)
                {
                    if (VirtualQuery((nint)cursor, out var region, (nuint)Marshal.SizeOf<MemoryBasicInformation>()) == 0) break;
                    Console.Error.WriteLine($"[LOADER][TRACE] mprotect.host base=0x{(ulong)region.BaseAddress:X} alloc=0x{(ulong)region.AllocationBase:X} len=0x{region.RegionSize:X} state=0x{region.State:X} prot=0x{region.Protect:X}");
                    var next = (ulong)region.BaseAddress + (ulong)region.RegionSize;
