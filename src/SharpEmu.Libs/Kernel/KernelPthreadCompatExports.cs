...
public static class KernelPthreadCompatExports
{
        ...
        bool posixErrors = false)
    {
        ...
        while (true)
        {
            // The primary executor has no cooperative continuation. A collector
            // can interrupt it while it waits for startup; deliver on this thread
            // with neither the condition nor mutex lock held, then continue waiting.
            var scheduler = GuestThreadExecution.Scheduler;
            scheduler?.PollPendingExceptions(ctx);
            lock (state.SyncRoot)
            {
                ...
                if (!timed)
                {
                    Monitor.Wait(state.SyncRoot, scheduler is null ? Timeout.Infinite : 10);
                    continue;
                }
                ...
                var slice = scheduler is not null && remaining > TimeSpan.FromMilliseconds(10)
                    ? TimeSpan.FromMilliseconds(10)
                    : remaining;
                Monitor.Wait(state.SyncRoot, slice);
            }
        }
