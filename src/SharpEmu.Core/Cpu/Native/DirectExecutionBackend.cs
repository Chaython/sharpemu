...
public sealed unsafe partial class DirectExecutionBackend : INativeCpuBackend, IGuestThreadScheduler, IDisposable
{
        ...
        // A long allocator pass can repeatedly call the same mutex imports
        // while making progress entirely in guest code. Automatic termination
        // on that pattern aborts healthy loading. Keep forced unwinding opt-in
        // through SHARPEMU_IMPORT_LOOP_GUARD_SECONDS; stall snapshots still run.
...
#pragma warning restore CS0649
        ...
        // Unresolved-import pacing: per-NID call counters that (a) throttle the
        // WARN log to the first occurrence plus periodic summaries and (b) pace
        // repeated calls with a host sleep once a caller is provably spinning
        // on an unimplemented import, so a missing export can no longer starve
        // every other guest thread of CPU (observed with _umtx_op: two worker
        // threads at 100% producing ~200k identical warnings and freezing the
        // game window).
        private readonly System.Collections.Concurrent.ConcurrentDictionary<string, long> _unresolvedImportCallCounts = new();
        ...
        public void RegisterGuestThreadContext(ulong threadHandle, CpuContext context)
        {
                ...
                lock (_guestThreadGate)
                {
                        ...
                        var stackRegion = TryGetVirtualMemory(context, out var memory)
                                ? memory.SnapshotRegions().FirstOrDefault(region =>
                                        stackPointer >= region.VirtualAddress &&
                                        stackPointer - region.VirtualAddress < region.MemorySize)
                                : default;
                        _externalGuestThreads[threadHandle] = new ExternalGuestThreadState
                        {
                                Context = context,
                                StackBase = stackRegion.VirtualAddress,
                                StackSize = stackRegion.MemorySize,
                        };
                }
        }

        public bool TryGetGuestThreadStack(ulong threadHandle, out ulong stackAddress, out ulong stackSize)
        {
                lock (_guestThreadGate)
                {
                        if (_guestThreads.TryGetValue(threadHandle, out var thread))
                        {
                                stackAddress = thread.StackBase;
                                stackSize = thread.StackSize;
                                return stackSize != 0;
                        }
                        if (_externalGuestThreads.TryGetValue(threadHandle, out var external))
                        {
                                stackAddress = external.StackBase;
                                stackSize = external.StackSize;
                                return stackSize != 0;
                        }
                }
                stackAddress = stackSize = 0;
                return false;
        }
                ...
                out string? error)
        {
                ...
                using var interruptedBlock = GuestThreadExecution.SuspendCurrentThreadBlock();
                try
                {
                        LastError = null;
        ...
        public void PollPendingExceptions(CpuContext callerContext)
        {
                // A dispatcher may inspect this context from another host thread.
                // Only its own executor can suspend it and run the guest handler.
                if (Volatile.Read(ref _pendingGuestExceptionCount) == 0 ||
                        !ReferenceEquals(_activeExecutionBackend, this) ||
                        !ReferenceEquals(_activeCpuContext, callerContext) ||
                ...
                DeliverPendingGuestExceptionAtSafePoint(
                        ...
                        CaptureImportBoundaryContinuation(
                                callerContext,
                                unchecked((nint)(frame.ResumeRsp - 104UL)),
                                frame.ReturnRip));
        }
        ...
        private unsafe static bool TryCaptureHostThreadContext(int hostThreadId, out HostThreadContextSnapshot snapshot)
        {
                ...
                // Allocate before suspending: the target may hold the native heap
                // lock. Allocating/freeing while it is suspended can deadlock the
                // sampler and leave the guest executor stopped indefinitely.
