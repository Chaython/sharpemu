        ...
        private unsafe ulong DispatchImport(int importIndex, nint argPackPtr)
        {
                ...
                try
                {
                        ...
                        if (!dispatchResolved)
                        {
                                ...
                                // Track per-NID unresolved call counts: the WARN log prints the
                                // first occurrence and then only periodic summaries, and a caller
                                // that is provably spinning (thousands of retries on the same
                                // missing import) is paced with a short host sleep so it can no
                                // longer starve every other guest thread of CPU. A healthy guest
                                // touches an unresolved import a handful of times; only a busy
                                // wait loop reaches the pacing threshold.
                                var unresolvedCount = _unresolvedImportCallCounts.AddOrUpdate(
                                        importStubEntry.Nid, 1, static (_, current) => current + 1);
                                ...
                                else if (unresolvedCount == 10_000 || (unresolvedCount % 1_000_000) == 0)
                                {
                                        Console.Error.WriteLine(
                                                $"[LOADER][WARN] Import unresolved x{unresolvedCount}: nid={importStubEntry.Nid} " +
                                                $"ret=0x{num7:X16} (repeated calls are being throttled and paced)");
                                }

                                if (unresolvedCount > 4096)
                                {
                                        // Pacing sleep: converts an unresolved-import busy loop into
                                        // a ~1 kHz poll that leaves the CPU to the guest threads that
