...
public sealed unsafe class PhysicalVirtualMemory : IVirtualMemory, IGuestMemoryAllocator, IGuestAddressSpace, IDisposable
{
    ...
    public bool TryProtect(ulong address, ulong size, GuestPageProtection protection)
    {
        ...
        try
        {
            ...
            if (!IsRangeCoveredByRegionsLocked(address, end))
            {
                return false;
            }

            // One guest mapping may span several Windows reservations. A single
            // VirtualProtect cannot cross their boundaries. Query also splits at
            // protection changes, so rollback preserves each page's original flags.
            var ranges = new List<(ulong Address, ulong Size, uint OldProtection)>();
            for (var cursor = address; cursor < end;)
            {
                if (!_hostMemory.Query(cursor, out var region) ||
                    region.State != HostRegionState.Committed ||
                    region.BaseAddress > cursor ||
                    region.RegionSize > ulong.MaxValue - region.BaseAddress ||
                    region.BaseAddress + region.RegionSize <= cursor)
                {
                    return false;
                }

                var length = Math.Min(end, region.BaseAddress + region.RegionSize) - cursor;
                ranges.Add((cursor, length, region.RawProtection));
                cursor += length;
            }
            ...
            for (var index = 0; index < ranges.Count; index++)
            {
                ...
                if (!_hostMemory.Protect(range.Address, range.Size, hostProtection, out var previous))
                {
                    for (var restoreIndex = index - 1; restoreIndex >= 0; restoreIndex--)
                    {
                        var restore = ranges[restoreIndex];
                        _hostMemory.ProtectRaw(restore.Address, restore.Size, restore.OldProtection, out _);
                    }

                    return false;
                }

                ranges[index] = (range.Address, range.Size, previous);
            }
