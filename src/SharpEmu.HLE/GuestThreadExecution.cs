...
public interface IGuestThreadScheduler
{
    ...
    /// <summary>Returns the pthread's allocated stack, including during callbacks.</summary>
    bool TryGetGuestThreadStack(ulong threadHandle, out ulong stackAddress, out ulong stackSize)
    {
    ...
    /// <summary>
    /// Services exceptions on the executor currently inside a host-side wait.
    /// Call outside synchronization locks; a guest handler can block until
    /// another guest thread acknowledges or resumes it.
    /// </summary>
    void PollPendingExceptions(CpuContext callerContext) { }
...
public static class GuestThreadExecution
{
    ...
    /// <summary>
    /// Keeps an interrupted import's pending wait out of a nested callback.
    /// Callback imports may block independently; the caller's request must be
    /// consumed only after the callback returns to the original dispatcher.
    /// Dispose on the same host thread, in reverse nesting order.
    /// </summary>
    public static PendingBlockScope SuspendCurrentThreadBlock()
    {
        var scope = new PendingBlockScope(
            _pendingBlockReason, _pendingBlockContinuationValid, _pendingBlockContinuation,
            _pendingBlockWakeKey, _pendingBlockWaiter, _pendingBlockDeadlineTimestamp);
        _pendingBlockReason = null;
        _pendingBlockContinuationValid = false;
        _pendingBlockContinuation = default;
        _pendingBlockWakeKey = null;
    ...
    public readonly struct PendingBlockScope : IDisposable
    {
        private readonly string? _reason;
        private readonly bool _hasContinuation;
        private readonly GuestCpuContinuation _continuation;
        private readonly string? _wakeKey;
        private readonly IGuestThreadBlockWaiter? _waiter;
        private readonly long _deadline;

        internal PendingBlockScope(string? reason, bool hasContinuation,
            GuestCpuContinuation continuation, string? wakeKey,
            IGuestThreadBlockWaiter? waiter, long deadline)
        {
            _reason = reason;
            _hasContinuation = hasContinuation;
        ...
        public void Dispose()
        {
            _pendingBlockReason = _reason;
            _pendingBlockContinuationValid = _hasContinuation;
            _pendingBlockContinuation = _continuation;
            _pendingBlockWakeKey = _wakeKey;
            _pendingBlockWaiter = _waiter;
            _pendingBlockDeadlineTimestamp = _deadline;
        }
    }
