...
// address-wait wake releases an umtx waiter and vice versa. Return values are
// raw FreeBSD 11 errnos (positive), matching the raw syscall convention the
// guest's libc wrappers expect; there is no ORBIS_GEN2 translation at this
// layer. Waits park the guest thread for real: a parked thread consumes no
...
public static class KernelUmtxCompatExports
{
    ...
    private const int ErrnoWouldblock = 35; // EWOULDBLOCK == EAGAIN
    ...
    public static int UmtxOp(CpuContext ctx)
    {
        ...
        return op switch
        {
            UmtxOpWait => DoWait(ctx, obj, val, uaddr, size: 8),
            UmtxOpWaitUint or UmtxOpWaitUintPrivate => DoWait(ctx, obj, val, uaddr, size: 4),
    ...
    //
    // Futex semantics: if the word at obj no longer equals val, return
    // EWOULDBLOCK immediately so the caller re-reads it; otherwise park until
    // the value differs, a matching wake arrives, or the deadline expires.
    private static int DoWait(CpuContext ctx, ulong address, ulong expected, ulong timeoutAddress, int size)
    {
        if (address == 0)
        {
            return ErrnoFault;
